...
pair's own enforced minimums. Kraken rejects an order whose price or volume
carries more decimal places than the pair allows, and separately rejects an
order below the pair's `ordermin`/`costmin` -- both are per-pair fields on
`kraken.client.asset_pairs()`'s response (`pair_decimals`, `lot_decimals`,
`ordermin`, `costmin`).

Ported from a parallel Kraken migration's independently-written account/
package (same underlying Kraken semantics, adapted here to this project's
pure-Python REST client instead of that version's kraken-cli dependency).
"""
